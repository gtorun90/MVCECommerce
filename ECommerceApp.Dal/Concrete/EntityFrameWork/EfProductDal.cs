    ...
    public class EfProductDal:IProductDal
    {
        ...
        public List<Product> GetAll()
        {
            return _context.Products.Where(p => p.IsHome).Select(p => new { p.Id, p.Name, p.Description, p.Image, p.Price, p.Stock, p.IsHome, p.IsApproved, p.CategoryId }).ToList()
                                                .Select(p => new Product() { Id = p.Id, Name = p.Name.Length > 100 ? p.Name.Substring(0, 100) + "..." : p.Name, Description = p.Description.Length > 50 ? p.Description.Substring(0, 50) + "..." : p.Description, Image = p.Image, Price = p.Price, Stock = p.Stock, IsHome = p.IsHome, IsApproved = p.IsApproved, CategoryId = p.CategoryId }).ToList(); ;
        }           
